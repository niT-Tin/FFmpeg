...
pub fn parse_sps(bit_reader: *BitReader) !SPS {
    var sps = SPS.init();
    ...
    sps.seq_parameter_set_id = try expGolomb.read_ue(bit_reader);

    if (has_high_ext) {
        const chroma_format_idc = try expGolomb.read_ue(bit_reader);
        if (chroma_format_idc == 3) {
            _ = try bit_reader.next_bit();
        }

        _ = try expGolomb.read_ue(bit_reader); // bit_depth_luma_minus8
        _ = try expGolomb.read_ue(bit_reader); // bit_depth_chroma_minus8
        ...
        if (scaling_present) {
            ...
            for (0..scaling_lists) |i| {
                ...
                if (flag) {
                    ...
                    for (0..list_size) |_| {
                        if (next_scale != 0) {
                            const delta = try expGolomb.read_se(bit_reader);
    ...
    sps.log2_max_frame_num_minus4 = try expGolomb.read_ue(bit_reader);
    sps.pic_order_cnt_type = try expGolomb.read_ue(bit_reader);

    if (sps.pic_order_cnt_type == 0) {
        sps.log2_max_pic_order_cnt_lsb_minus4 = try expGolomb.read_ue(bit_reader);
    } else if (sps.pic_order_cnt_type == 1) {
        sps.delta_pic_order_always_zero_flag = try bit_reader.next_bit() != 0;
        sps.offset_for_non_ref_pic = try expGolomb.read_se(bit_reader);
        sps.offset_for_top_to_bottom_field = try expGolomb.read_se(bit_reader);
        sps.num_ref_frames_in_pic_order_cnt_cycle = try expGolomb.read_ue(bit_reader);
        for (0..sps.num_ref_frames_in_pic_order_cnt_cycle) |i| {
            sps.offset_for_ref_frame[i] = try expGolomb.read_se(bit_reader);
        }
    }

    sps.num_ref_frames = try expGolomb.read_ue(bit_reader);
    sps.gaps_in_frame_num_value_allowed_flag = try bit_reader.next_bit() != 0;
    sps.pic_width_in_mbs_minus1 = try expGolomb.read_ue(bit_reader);
    sps.pic_height_in_map_units_minus1 = try expGolomb.read_ue(bit_reader);
    ...
    if (sps.frame_cropping_flag) {
        sps.frame_crop_left_offset = try expGolomb.read_ue(bit_reader);
        sps.frame_crop_right_offset = try expGolomb.read_ue(bit_reader);
        sps.frame_crop_top_offset = try expGolomb.read_ue(bit_reader);
        sps.frame_crop_bottom_offset = try expGolomb.read_ue(bit_reader);
    }
